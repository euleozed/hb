...
df = df.sort_values(by=['Processo', 'Data/Hora']).set_index('Processo')
...
if processo_selecionado:
    ...
    df_table = df_selected[['Data/Hora', 'Unidade', 'Protocolo', 'Documento', 'Data Documento', 'Dias entre Documentos', 'Dias Acumulados']].sort_values(by='Data/Hora', ascending=False)
    ...
    df_qtd_documentos = df_selected[(df_selected['Unidade'] == 'SESAU-HB') &
    ...
    c1.metric("Quantidade de Documentos Produzidos pela GAD HB:", qtd_documentos)
    ...
    fig_tempo_maximo.update_layout(
        polar=dict(
            ...
            radialaxis=dict(
                visible=False,
                # gridcolor="red",  # Cor das linhas do eixo radial em vermelho
                # linecolor="red"   # Cor das linhas do eixo em vermelho
            ),
            angularaxis=dict(
                # gridcolor="red",  # Cor das linhas do eixo angular em vermelho
                # linecolor="red"   # Cor das linhas do eixo em vermelho
            )
        ),
        showlegend=False
...
#     # Tabela de tempo de servidor HB
#     st.markdown(f"<h5 style='text-align: center;'>Análise Temporal dos Servidores HB</h5>", unsafe_allow_html=True)
...
# # Carregar dataframe "df_atrasados" para outra página com session state
# st.session_state['df_atrasados'] = df_atrasados
# st.write("DataFrame armazenado no session_state:")
